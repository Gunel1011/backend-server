...
// 1. Köməkçi Funksiya (Bunu DƏYİŞMƏYİN, olduğu kimi qalsın yuxarıda)
const sendOTPToEmail = (email, otp) => {
  ...
  const mailOptions = {
    ...
    to: email,
    subject: "OTP Code", // Və ya istədiyiniz başlıq
...
// 2. Controller Funksiyası (Bunun adını 'sendAdminDeleteOTP' qoyuruq)
const sendAdminDeleteOTP = async (req, res) => {
...
// Sonda: module.exports = { ..., sendAdminDeleteOTP }
...
const adminChangeStatus = async (req, res) => {
  ...
  try {
    ...
    user.isActive = isActive; // Statusu dəyişirik (true/false)
    await user.save();

    res.send("Status dəyişdirildi.");
  } catch (error) {
    res.status(500).send("Xəta: " + error.message);
  }
};

// module.exports-a əlavə etməyi UNUTMAYIN!
// module.exports = { ..., adminChangeStatus }
const adminDeactivateUser = async (req, res) => {
  const token = req.headers.authorization;
  const { email, isActive } = req.body; // Frontend-dən gələn məlumat
...
const adminDeleteUser = async (req, res) => {
  const token = req.headers.authorization;
  const { userId, otp } = req.body; // OTP də qəbul edirik
  ...
  try {
    ...
    // OTP yoxlanışı
    if (admin.otp !== otp) {
      return res.status(400).send("OTP kodu yanlışdır!");
    }

    // OTP düzdürsə, silirik və OTP-ni sıfırlayırıq
    admin.otp = null;
    await admin.save();

    await User.findByIdAndDelete(userId);
    res.send({ message: "İstifadəçi uğurla silindi" });
  } catch (error) {
    res.status(500).send("Xəta: " + error.message);
  }
};

// ... module.exports hissəsinə 'sendAdminDeleteOTP' funksiyasını da ƏLAVƏ EDİN.
// module.exports = { ..., adminDeleteUser } etməyi UNUTMAYIN!
